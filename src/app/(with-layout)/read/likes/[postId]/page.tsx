...
import { fetchPostLikes } from "@/actions/fetchsAction"
...
const LikesPage = async ({ params, searchParams }: { params: Promise<{ postId: string }>, searchParams: Promise<{ page: number, limit: number }> }) => {
  const {postId} = await params;
  const { page, limit } = await searchParams;
  const {data, message, redirectUrl, success, meta} = await fetchPostLikes(postId, page, limit);
  ...
  return (
    <div className="grow flex flex-col mt-4 text-black w-full">
      <h3 className="text-xl font-bold self-center mb-10">Users who liked your post</h3>
