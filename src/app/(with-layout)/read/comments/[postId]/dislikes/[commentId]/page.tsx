...
import { fetchCommentsDislikes } from '@/actions/fetchsAction';
...
const CommentsLikesPage = async ({ params, searchParams }: { params: Promise<{commentId: string, postId: string}>, searchParams: Promise<{ page: number, limit: number }> }) => {
  ...
  const {data, message, redirectUrl, success, meta} = await fetchCommentsDislikes(postId, commentId, page, limit);
  ...
  return (
    <div className="grow flex flex-col mt-4 text-black w-full">
      <h3 className="text-xl font-bold self-center mb-10">Users who disliked the comment</h3>
