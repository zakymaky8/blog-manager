...
import { fetchCommentsLikes } from '@/actions/fetchsAction';
...
const CommentsLikesPage = async ({ params, searchParams }: { params: Promise<{commentId: string, postId: string}>, searchParams: Promise<{ page: number, limit: number }> }) => {
  const { commentId, postId } = await params;
  const { page, limit } = await searchParams;
  const {data, message, redirectUrl, success, meta} = await fetchCommentsLikes(postId, commentId, page, limit);
  ...
  return (
    <div className="grow flex flex-col mt-4 text-black w-full">
      <h3 className="text-xl font-bold self-center mb-10">Users who liked the comment</h3>
      <ul className="flex flex-col gap-1 items-center mb-40">
        ...
        }) : <span className="opacity-70 italic mt-6">No user has liked yet!</span>
      }
      </ul>
      <Pagination
            type="reactions"
...
export default CommentsLikesPage