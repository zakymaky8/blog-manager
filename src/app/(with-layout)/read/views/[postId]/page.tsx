...
import { fetchPostViews } from "@/actions/fetchsAction"
...
const ViewsPage = async ({ params, searchParams }: { params: Promise<{ postId: string }>, searchParams: Promise<{ page: number, limit: number }> }) => {
  ...
  return (
    <div className="grow flex flex-col mt-4 text-black w-full">
      <h3 className="text-xl font-bold self-center mb-10">Users who viewed your post</h3>
      <ul className="flex flex-col gap-1 items-center mb-40">
        ...
        }) : <span className="opacity-70 italic mt-6">No user has viewed yet!</span>
      }
      </ul>
      <Pagination
            type="reactions"
